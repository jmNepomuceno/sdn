...
$(document).ready(function(){
    ...
    $('#check-if-registered-div').on('mouseleave', function() {
    ...
    $('#yes-modal-btn').off('click', '#yes-modal-btn').on('click' , function(event){
        ...
        if($('#yes-modal-btn').text() == 'Yes'){
            ...
            data = {
                ...
                hospital_code : parseInt($('#hperson-hospital-no').text()),
            ...
            for(let i = 0; i < all_non_req_input_arr.length; i++){
                ...
                // console.log($(all_non_req_input_arr[i]).val() === "" && $(all_non_req_input_arr[i]).prop('tagName').toLowerCase() !== 'span')
                // console.log($(all_non_req_input_arr[i]).prop('tagName').toLowerCase())
