$(document).ready(function(){
    let current_page = ""
    let table_index = 0
    let prompt_modal = new bootstrap.Modal(document.getElementById('myModal-prompt'));
    // prompt_modal.show()
    //----------------------------------------------------------------------------
    ...
    // add-doctor-btn
    $('#add-doctor-btn').on('click' , function(event){
        const data = {
            last_name : $('#input-lname').val(),
            first_name : $('#input-fname').val(),
            middle_name : $('#input-mname').val(),
            mobile : $('#input-mnum').val(),
            action : "add"
        }
        console.log(data)
        $.ajax({
            url: '../SDN/add_doctor_list.php',
            data : data,                        
            method: "POST",
            success: function(response) {
              prompt_modal.show()
              $('.doctor-container').html("")
              $('.doctor-container').html(response)
            }
        });
    })

    // remove-doctor-btn
    $(document).on('click', '.remove-doctor-btn', function(event){
      const index = $('.remove-doctor-btn').index(this);
      console.log(index)
      const data = {
        index : $('.mobile-num-h3').eq(index).text(),
        action : 'delete'
      }
