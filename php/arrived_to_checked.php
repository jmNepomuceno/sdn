<?php
    ...
    $checkup_textarea = filter_input(INPUT_POST, 'checkup_textarea');
    ...
    $sql = "UPDATE incoming_referrals SET status='Checked', checkup_details=:checkup_textarea, checkup_time=:timer WHERE hpercode=:hpercode AND refer_to = '" . $_SESSION["hospital_name"] . "'";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':timer', $currentDateTime, PDO::PARAM_STR);
    $stmt->bindParam(':checkup_textarea', $checkup_textarea, PDO::PARAM_STR); // currentDateTime
