    ...
    <div class="flex flex-row justify-start items-center w-full h-full"> 
        ...
        <div class="w-full h-full border flex flex-col justify-start items-start overflow-hidden">
            <div class="w-full h-[5%] ml-[3%]  flex flex-row justify-start items-center border-t-0  rounded-md">
                <label class="text-5xl font-serif text-[#333333] mt-4">Dashboard For Outgoing Referrals</label>
            </div>
                    ...
                    <tbody class="w-full">
                        ...
                        <?php 
                            ...
                            for($i = 0; $i < count($refer); $i++){
                                echo '<input type="hidden" class="referred-to-class" value="' . $refer[$i]["refer_to"] . '">';
                            }
                            ...
                            $sql = "SELECT pat_class, type, refer_to FROM incoming_referrals WHERE (status='Approved' OR status='Checked' OR status='Arrived') AND referred_by = '" . $_SESSION["hospital_name"] . "'";
                            $stmt = $pdo->prepare($sql);
                            $stmt->execute();
